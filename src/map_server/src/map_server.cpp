...
{	
	...
	ros::Publisher map_pub = n.advertise<OccupancyGrid>("map", 10);
	Mat image = imread("src/map_server/src/" + file_name, IMREAD_UNCHANGED); 
	if(!image.data ) {
	...
	OccupancyGrid msg;
	msg.data.reserve(image_bw.rows * image_bw.cols);
	for (int x = image_bw.cols - 1; x >= 0; x--) {
  		for (int y = image_bw.rows - 1; y >= 0; y--) {
  			const unsigned char intensity = image_bw.at<unsigned char>(y, x);
    			msg.data.push_back(intensity);
  		}
	}		
	msg.header.seq = 1;
...
//If you were to take a look at the published message in rostopic, you would see a bunch of -1s among the 0s. This is not shocking; the white points are stored as 255, which in unsigned 8bit binary would be 11111111. However, ros reads it as 2's complement, and 11111111 is -1. However, as long as we humans know the true meaning, it shouldnt make a difference. "Shouldnt"
		...
		namedWindow( file_name, cv::WINDOW_AUTOSIZE );
		imshow( file_name, image_bw );
